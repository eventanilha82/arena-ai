...
Depois dos 16 avos, os 16 placares de tempo regulamentar são acrescentados ao mesmo posterior Gamma-Poisson, mantendo congelado o prior de `8,0` gols equivalentes escolhido antes desses jogos. Prorrogação e cobranças de pênaltis continuam separadas: elas resolvem a chave, mas não atualizam uma taxa cujo alvo é 90 minutos. O peso mediano da forma passa de `34,6%` após os grupos para `40,0%` após os 16 avos. Nas oito partidas seguintes, essa atualização desloca a probabilidade de avanço em no máximo `0,9` ponto percentual; o gate reprova qualquer atualização de uma rodada que ultrapasse `5 p.p.` em um confronto.

`make bolao-knockout-audit` preserva os 16 avos como teste externo da forma dos grupos. Na foto de 2026-07-04, a forma melhora todas as perdas auditadas em relação ao histórico, mas o ganho é pequeno: log-loss 1X2 de `0,8546` para `0,8500`, Brier de `0,4961` para `0,4937`, log-loss de placar de `2,4181` para `2,3789` e log-loss de avanço de `0,4663` para `0,4648`. Cinco dos 16 jogos empataram em 90 minutos contra `3,77` esperados; o resíduo padronizado de `0,73` não sustenta uma recalibração de empate. A amostra também não sustenta retreino do XGBoost, mudança dos pesos globais ou habilidade própria de pênaltis. O prior `8,0` está na borda superior da grade original; como janelas temporais mais amplas não indicaram um substituto estável, ele fica congelado e explicitamente sinalizado, em vez de ser retunado retrospectivamente nos 16 avos.

`make bolao-round16-audit` usa a foto após os 16 avos como candidata prospectiva e as oito oitavas como novo teste externo. O histórico puro ficou ligeiramente melhor: log-loss 1X2 de `0,7766` contra `0,7834`, Brier de `0,4383` contra `0,4430`, log-loss de placar de `3,0051` contra `3,0091` e log-loss de avanço de `0,5028` contra `0,5114`. As cinco diferenças de perda permanecem abaixo do guardrail operacional de `0,01`, reutilizado do limiar de materialidade temporal já existente; isso não é uma alegação formal de não inferioridade. Com apenas oito jogos, o resultado não sustenta retreino, troca do prior ou desligamento abrupto da forma. Os oito placares regulamentares são acrescentados ao posterior, o peso mediano passa de `40,0%` para `41,9%` e o maior deslocamento de avanço nas quartas é `0,62 p.p.`. O único empate em 90 minutos ficou dentro de dois desvios-padrão da expectativa, e a única disputa não altera a política neutra de pênaltis.

`make bolao-quarterfinal-audit` congela a foto após as oitavas e usa as quatro quartas como teste prospectivo. A forma identifica os quatro classificados, reduz marginalmente log-loss 1X2 (`0,9099` para `0,9089`) e Brier (`0,5576` para `0,5557`), mas fica ligeiramente pior em log-loss de placar (`2,1852` para `2,1858`), log-loss de avanço (`0,3056` para `0,3129`) e Brier de avanço (`0,0714` para `0,0742`). Todas as diferenças permanecem dentro do guardrail operacional de `0,01`; com apenas quatro jogos dependentes, isso é evidência descritiva, não seleção de hiperparâmetros nem teste formal de não inferioridade. Os dois empates regulamentares ficaram a `1,16` desvios-padrão da expectativa. Os três gols de prorrogação não entram na taxa de 90 minutos; o peso mediano passa de `41,9%` para `42,3%` e o maior delta de avanço nas semifinais é `0,32 p.p.`. A política, o prior `8,0`, o híbrido e os pênaltis neutros permanecem congelados.

`make bolao-final-audit` usa três snapshots congelados: após as quartas para os jogos 101–102, após as semifinais para o jogo 103 e após o terceiro lugar para o jogo 104. A forma identifica Espanha e Argentina como finalistas; o placar modal de Inglaterra `1 x 2` Argentina é exato, e a Espanha lidera o Monte Carlo com `35,4%` antes das semifinais e `57,9%` antes da final. Nos quatro jogos, a acurácia de avanço é `75%`: o erro é o terceiro lugar França `4 x 6` Inglaterra, ao qual a matriz atribuía `0,0069%` de probabilidade exata. A final termina `0 x 0` em 90 minutos, mas a forma atribui `57,1%` de avanço à Espanha e acerta a campeã. Agregadas, as cinco diferenças de perda contra o histórico permanecem dentro do guardrail de `0,01`; isso não transforma quatro jogos em evidência de retreino. Como a matriz processada não preserva a fase histórica, o projeto não consegue estimar um regime próprio de terceiro lugar e não adiciona multiplicador retrospectivo. O prior `8,0` permanece congelado, o peso mediano encerra em `43,4%` e o gol da prorrogação fica fora da taxa regulamentar.

`make bolao-top10-audit` gera `bolao_top10_bias_audit.json/.csv`. Para cada candidato do top 10, ele compara a campanha com e sem a forma atual, confronta o time contra todos os demais classificados nas duas ordens nominais e exige: simetria de 1X2/xG/avanço, probabilidades normalizadas, pênaltis 50/50 e exclusão de seleções eliminadas por resultados observados. Enquanto ainda há jogos futuros, os intervalos de título medem apenas o erro de amostragem Monte Carlo. Depois da final, o relatório mantém a auditoria de simetria, mas rotula a Espanha como desfecho observado e deixa esses intervalos sem valor; repetir a chave encerrada não estima incerteza preditiva.
